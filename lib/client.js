...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * @returns the measured width in CSS pixels, or 0 before it is known.
     */
    function useMeasuredWidth(ref) {
      ...
      React.useEffect(() => {
        ...
        const measure = () => {
          ...
          setWidth(rect !== undefined && rect !== null && rect.width > 0 ? rect.width : 0);
        };
        measure();
    ...
    function Heatmap(props) {
      ...
      const tip = useTip();
      const availableWidth = useMeasuredWidth(tip.wrap);
    ...
    function WeeklyBars(props) {
      ...
      const measured = useMeasuredWidth(tip.wrap);
    ...
    function createPanel(ctx, seams) {
      ...
      return function Panel() {
        ...
        const load = (force) => {
          setBusy(true);
          hostCall(force ? 'refresh' : 'summary', {}).then((result) => {
            setBusy(false);
        ...
        if (model !== null) {
          ...
          body = h('div', { className: 'dts-body' },
            ...
            h('div', { className: 'dts-section' },
              ...
              view === 'daily'
                ? h(Heatmap, { t, model, metric })
                : h(WeeklyBars, { t, model, metric })),
