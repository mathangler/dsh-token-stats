...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    const CELL_TARGET = 30;
    const CELL_GAP = 3;
    const GUTTER = 18;
    /** A square wider than this reads as a slab, not as a day. */
    const CELL_MAX = 34;
    /** Rows in the daily grid, and the month-label strip above them. */
    const HEAT_ROWS = 7;
    const HEAT_HEADER = 16;
    /** Window bounds: a quarter of a year at the narrowest, half a year at most. */
    const MIN_WEEKS = 12;
    const MAX_WEEKS = 26;
     ...
     *
     * The window follows the card width so the blocks stay one legible size
     * instead of being squeezed as the card narrows, but it is bounded on both
     * ends by what is worth reading: a quarter of a year at the narrowest, and
     * half a year at the widest. A year-long grid forced each block down to
     * ~19px, and the oldest months of it say nothing about how the machine is
     * being used now. See {@link gridGeometry} for the size that follows.
     *
    ...
    function heatmapWeeks(availableWidth) {
      ...
      const weeks = Math.round((width - GUTTER) / (CELL_TARGET + CELL_GAP));
     ...
     *
     * One block, shared by the grid and the bars: the same week count, the same
     * column pitch, the same width and the same height, so the two views are the
     * same object seen two ways and switching between them moves nothing — and,
     * because both place column i at the same x, a week sits exactly under its
     * seven days.
     *
     * The cell is whatever divides the width evenly, capped so a very wide card
     * cannot turn 26 weeks into 50px slabs; past the cap the block is centred
     * rather than stretched. The stylesheet's heat grid is `gutter | column ×
     * weeks` with a 3px gap between every pair, which is what the formula below
     * mirrors.
     *
     * @param availableWidth - measured slot width in CSS pixels, 0 when unknown.
     * @returns `{ weeks, cell, pitch, gutter, gap, gridWidth, height }` in CSS pixels.
     */
    function gridGeometry(availableWidth) {
      const width = availableWidth > 0 ? availableWidth : 820;
      const weeks = heatmapWeeks(width);
      const cell = Math.min(CELL_MAX, Math.max(6, (width - GUTTER - CELL_GAP * weeks) / weeks));
    ...
    function WeeklyBars(props) {
      ...
      // first column starts one gap after the gutter. A bar is exactly as wide as
      // a day square.
      const colLeft = (i) => geometry.gutter + geometry.gap + i * geometry.pitch;
      const barWidth = geometry.cell;
      ...
      return chartShell(tip, h('div', { className: 'dts-heat-center' },
        h('svg', {
          ...
          // apart before.
          style: { width: width + 'px', flex: 'none' },
          viewBox: '0 0 ' + width + ' ' + height,
          role: 'img',
        },
        h('line', { className: 'dts-grid', x1: colLeft(0), y1: bottom, x2: colLeft(weeks.length - 1) + barWidth, y2: bottom }),
