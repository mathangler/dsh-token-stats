...
async function saveDurable(state, options) {
  ...
  const body = JSON.stringify({ version: CACHE_VERSION, savedAt: Date.now(), sessions });
...
export function createHandlers(ctx, options = {}) {
  ...
  /**
   * Serve one aggregate, coalescing concurrent callers onto a single scan.
  ...
  async function summary(args) {
    ...
    if (fresh && !force) return state.aggregate;
    if (state.inFlight !== null) return state.inFlight;
    const pending = collect(ctx, state, resolved).then(
      (value) => {
